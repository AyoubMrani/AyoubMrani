# 👋 Hi, I'm Ayoub Mrani Alaoui

## 🤖 AI & Data Science Specialist

Building intelligent systems that solve real-world problems.

### 🎯 Expertise
- **Machine Learning:** Scikit-learn, TensorFlow, PyTorch
- **NLP & Information Retrieval:** RAG Systems, Hybrid Search, Ranking Fusion (RRF)
- **Full-Stack Development:** FastAPI, React, Django, Laravel
- **Data Science:** Python, SQL, Data Pipeline, Feature Engineering
- **Cloud & DevOps:** Docker, Git, Linux

### 🔥 Featured Project: RAG Incident Resolution System
Designed and evaluated a **hybrid dense/BM25 retriever** for IT incident resolution
- **Recall@5:** 1.000 vs 0.768 (dense-only)
- **Architecture:** Locally-hosted, API-free, production-grade
- **Stack:** Sentence-BERT, FastAPI, React, SQLite

[Read Full Report →](https://github.com/AyoubMrani/rag-incident-resolution)

### 🎓 Education
- **Master's Degree** in Data Science & AI — ENS Tétouan (2024–Current)
- **Professional Bachelor's** in Software & Web Development — ENS Tétouan (2023–2024)
- **Specialized Technician Diploma** in Digital Development — OFPPT (2021–2023)

### 📫 Let's Connect
- 📧 Email: ayoubmranialaoui@gmail.com
- 💼 LinkedIn: [Ayoub Mrani Alaoui](https://linkedin.com/in/ayoub-mrani-alaoui-1b5aa7237)
- 📱 Phone: (+212) 619960100
- 📍 Location: Fez, Morocco

### 💡 Open To
- Collaborating on ML/AI projects
- Data science consulting
- Full-stack development opportunities
- Knowledge sharing & mentoring

---

*"Building the future of AI, one model at a time."* 🚀
